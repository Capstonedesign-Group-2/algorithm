    ...
    clusters.forEach((cluster, i) => {
      ...
      if (cluster.length) {
        chart.data.datasets[0].data[i] = {
          x: Number((sumX / cluster.length).toFixed(2)),
          y: Number((sumY / cluster.length).toFixed(2))
        }
      }
    })
  ...
  return new Promise(async (resolve, reject) => {
    dataSet().forEach((dataPoint, i) => {
      ...
      for (let j = 0; j < k; j++) {
        ...
        disatnces.push(Math.sqrt(
          ((clusterCenterX - dataPoint.WGB) ** 2) + ((clusterCenterY - dataPoint.RGB) ** 2)
        ))
      }
      const minValue = Math.min.apply(Math, disatnces)
      const index = disatnces.indexOf(minValue)
      chart.data.datasets[0].pointBackgroundColor[i + k]
